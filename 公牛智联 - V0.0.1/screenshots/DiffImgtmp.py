...
class DiffImg(object):
    def result(self, temp_file, original):
        self.TEMP_FILE = temp_file
        self.get_screenshot_by_custom_size(40, 1570, 1040, 1750)
        results = self.same_as(r"ORIGINAL_PICTURE/%s" % original, 100)
        return results

    def same_as(self, load_image, percent):
        # 对比图片，percent值设为0，则100%相似时返回True，设置的值越大，相差越大
        image1 = Image.open(r"tmp.png")
    ...
    def get_screenshot_by_custom_size(self, start_x, start_y, end_x, end_y):
        ...
        newImage.save(r"tmp.png")


DiffImg().result("1 - 1 - 1898 - [GNAPP_LOGIN_007]-[2017-04-17 13_05_33].png",
                 "login_password_mistake.png")
