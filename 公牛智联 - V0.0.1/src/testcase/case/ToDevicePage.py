    ...
    def case(self, devices):
        ...
        while True:
            ...
            if self.driver.current_activity == login_page["activity"][0]:
                device[devices]["logger"].info(u"[APP_INF] APP当前页面为登录页面,登录")
                try:
                    ...
                    device[devices]["logger"].info(u"[APP_INF] APP进入设备主页失败，退出")
                    self.driver.quit()

            if self.driver.current_activity == device_page["activity"][0]:
                device[devices]["logger"].info(u"[APP_INF] APP当前页面为主页面")
                break
