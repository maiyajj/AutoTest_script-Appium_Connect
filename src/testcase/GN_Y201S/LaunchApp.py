...
from src.testcase.GN_Y201S.ToDevicePage import *
from src.testcase.GN_Y201S.ToLoginPage import *
        ...
        for ports in [self.port]:
            ...
            if not proc_pid:  # 判断当前端口是否被占用
                ...
                for i in proc_pid:
                    ...
                    except PidTerminalError:
                        self.debug.info("Kill %s error!" % i[0])
        ...
        while True:
            try:
                if strong_reboot is True:  # 重置端口后再启动Appium服务
                    if self.udid in self.sc.get_phone_udid():  # 判断手机是否还跟PC保持正常连接
