...
class GNF1331Timer5(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"定时器(#9)"  # 用例所属模块
        self.case_title = u'在线状态，多层循环定时执行状态检查'  # 用例名称
        self.zentao_id = "134"  # 禅道ID
    ...
    def case(self):
        ...
        tmp = self.create_cycle_timer("up_timer_page", now, time_1, time_2, u"5次")
        ...
        # 中层
        # 设置
        set_timer = set_cycle_dict[start_time_3]
        s_time, s_id, s_times = set_timer[0], set_timer[1], set_timer[2]
        result = [s_time is not None,
                  s_times == "5"]
        if False in result:
