...
class GNF1331Timer17(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"定时器(#9)"  # 用例所属模块
        self.case_title = u'在线状态，4组开与3组关按自定义方式执行的普通定时执行状态检查'  # 用例名称
        self.zentao_id = "90"  # 禅道ID
    ...
    def case(self):
        ...
        start_time_1, set_time_1 = self.create_normal_timer("up_timer_page", now, time_1, "power_on", loop=loop)
        start_time_2, set_time_2 = self.create_normal_timer("up_timer_page", now, time_2, "power_off", loop=loop)
        start_time_3, set_time_3 = self.create_normal_timer("up_timer_page", now, time_3, "power_on", loop=loop)
        start_time_4, set_time_4 = self.create_normal_timer("up_timer_page", now, time_4, "power_off", loop=loop)
        start_time_5, set_time_5 = self.create_normal_timer("up_timer_page", now, time_5, "power_on", loop=loop)
        start_time_6, set_time_6 = self.create_normal_timer("up_timer_page", now, time_6, "power_off", loop=loop)
        start_time_7, set_time_7 = self.create_normal_timer("up_timer_page", now, time_7, "power_on", loop=loop)
