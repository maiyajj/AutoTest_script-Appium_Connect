...
class GNF1331Timer4(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"定时器(#9)"  # 用例所属模块
        self.case_title = u'设备当前状态和循环定时输出状态相同，循环定时执行状态检查'  # 用例名称
        self.zentao_id = "139"  # 禅道ID
    ...
    def case(self):
        ...
        #####
        # 开关
        btn_dict = self.check_button_state(start_time_1, set_time_1, control_time_1, set_time_2)
        # 定时设置
