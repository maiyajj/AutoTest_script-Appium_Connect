...
class GNF1331Timer19(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"定时器(#9)"  # 用例所属模块
        self.case_title = u'在线状态，4组开与3组关按工作日方式执行的普通定时执行状态检查'  # 用例名称
        self.zentao_id = "88"  # 禅道ID
    ...
    def case(self):
        ...
        start_time_4, set_time_4 = self.create_normal_timer("up_timer_page", now, time_4, "power_off", loop=u"工作日")
        start_time_5, set_time_5 = self.create_normal_timer("up_timer_page", now, time_5, "power_on", loop=u"工作日")
        start_time_6, set_time_6 = self.create_normal_timer("up_timer_page", now, time_6, "power_off", loop=u"工作日")
        start_time_7, set_time_7 = self.create_normal_timer("up_timer_page", now, time_7, "power_on", loop=u"工作日")
