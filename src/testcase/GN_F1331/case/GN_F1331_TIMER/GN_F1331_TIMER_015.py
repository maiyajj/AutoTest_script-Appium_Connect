...
class GNF1331Timer15(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"定时器(#9)"  # 用例所属模块
        self.case_title = u'在线状态，随机各层设置9组普通定时，工作日执行的定时执行状态检查'  # 用例名称
        self.zentao_id = "96"  # 禅道ID
    ...
    def case(self):
        ...
        start_time_3, set_time_3 = self.create_normal_timer("up_timer_page", now, time_3, "power_on", loop=u"工作日")
        ...
        start_time_4, set_time_4 = self.create_normal_timer("mid_timer_page", now, time_4, "power_on", loop=u"工作日")
        start_time_5, set_time_5 = self.create_normal_timer("mid_timer_page", now, time_5, "power_off", loop=u"工作日")
        start_time_6, set_time_6 = self.create_normal_timer("mid_timer_page", now, time_6, "power_on", loop=u"工作日")
        ...
        start_time_7, set_time_7 = self.create_normal_timer("down_timer_page", now, time_7, "power_on", loop=u"工作日")
        start_time_8, set_time_8 = self.create_normal_timer("down_timer_page", now, time_8, "power_off", loop=u"工作日")
        start_time_9, set_time_9 = self.create_normal_timer("down_timer_page", now, time_9, "power_on", loop=u"工作日")
        ...
        set_normal_dict = self.check_set_normal_timer(start_time_1, start_time_2, start_time_3, start_time_4,
                                                      ...
                                                      start_time_9)
        # 定时执行
        launch_normal_dict = self.check_launch_normal_timer(set_normal_dict, set_time_1, set_time_2, set_time_3,
                                                            set_time_4, set_time_5, set_time_6, set_time_7,
                                                            set_time_8, set_time_9)
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_3]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_4]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_5]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_6]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_7]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_8]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
        ...
        if False in result:
            raise TimeoutException("device state error, current: %s, result: %s" % (s_time, result))
        # 执行
        launch_timer = launch_normal_dict[set_time_9]
        l_time, l_id = launch_timer[s_id][0], launch_timer[s_id][1]
        result = [l_time is not None,
                  l_id == s_id]
        if False in result:
