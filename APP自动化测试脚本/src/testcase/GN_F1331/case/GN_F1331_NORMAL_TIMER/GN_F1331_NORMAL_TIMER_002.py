...
class GNF1331NormalTimer2(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"APP功能测试"  # 用例所属模块
        self.case_title = u'上、中层循环定时'  # 用例名称
    ...
    def case(self):
        ...
        tmp = self.create_cycle_timer("up_timer_page", now, delay_time_1, delay_time_2, u"永久循环", delay_s=90)
        ...
        tmp = self.create_cycle_timer("mid_timer_page", now, delay_time_3, delay_time_4, u"永久循环")
        ...
        self.debug.info(u"[APP_INFO]device state: \n"
                        u"btn_state_list: %s;\n"
                        u"set_timer_list: %s;\n"
                        u"launch_timer_on_list: %s;\n"
                        u"launch_timer_off_list: %s;\n"
                        u"timer_list: %s;\n"
                        u"result: %s;"
                        % (btn_state_list, set_cycle_timer_list, launch_cycle_timer_on_list,
                           launch_cycle_timer_off_list, timer_list, result))

        up_cycle_id, mid_cycle_id = timer_list  # timer id
        up_launch_cycle_timer_on_list, mid_launch_cycle_timer_on_list = \
            result[up_cycle_id]["a"], result[mid_cycle_id]["a"]
        up_launch_cycle_timer_off_list, mid_launch_cycle_timer_off_list = \
            result[up_cycle_id]["b"], result[mid_cycle_id]["b"]
        self.debug.info(u"[APP_INFO]timer_list: %s;\n"
                        u"up_launch_cycle_timer_on_list: %s;\n"
                        u"mid_launch_cycle_timer_on_list: %s;\n"
                        u"up_launch_cycle_timer_off_list: %s;\n"
                        u"mid_launch_cycle_timer_off_list: %s;\n"
                        % (timer_list, up_launch_cycle_timer_on_list, mid_launch_cycle_timer_on_list,
                           up_launch_cycle_timer_off_list, mid_launch_cycle_timer_off_list))
        # 上层
        # 设置
