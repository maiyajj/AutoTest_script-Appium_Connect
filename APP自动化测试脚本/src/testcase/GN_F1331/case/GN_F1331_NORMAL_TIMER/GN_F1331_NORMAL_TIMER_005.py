...
class GNF1331NormalTimer5(WidgetOperation):
    @case_run(False)
    def run(self):
        self.case_module = u"APP功能测试"  # 用例所属模块
        self.case_title = u'上、中层延迟定时'  # 用例名称
    ...
    def case(self):
        ...
        self.debug.info(u"[APP_INFO]device state: \n"
                        ...
                        u"timer_list: %s;\n"
                        u"result: %s;"
                        % (btn_state_list, set_delay_timer_list, launch_delay_timer_list, timer_list, result))

        up_delay_id, mid_delay_id = timer_list  # timer id
        up_launch_delay_timer_list, mid_launch_delay_timer_list = \
            result[up_delay_id]["a"], result[mid_delay_id]["a"]
        self.debug.info(u"[APP_INFO]timer_list: %s;\n"
                        u"up_launch_delay_timer_list: %s;\n"
                        u"mid_launch_delay_timer_list: %s;"
                        % (timer_list, up_launch_delay_timer_list, mid_launch_delay_timer_list))
        # 上层
        # 设置
