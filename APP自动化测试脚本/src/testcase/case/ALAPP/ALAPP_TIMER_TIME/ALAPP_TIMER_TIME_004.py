...
class ALAppTimerTime4(WidgetOperationAL):
    @case_run(False)
    def run(self):
        self.case_module = u"FUT_MTIMER_TIME热水器模式_小夜灯_取暖器模式(#58)"  # 用例所属模块
        self.case_title = u'FUT_MTIMER_TIME_循环定时每周一循环'  # 用例名称
        self.zentao_id = 444  # 禅道ID
    ...
    def case(self):
        ...
        for elem, page, mode_info in timer_list:
            ...
            tmp, cycle = self.create_point_mode_timer(page, now, delay_time_1, delay_time_2, u"周一")
