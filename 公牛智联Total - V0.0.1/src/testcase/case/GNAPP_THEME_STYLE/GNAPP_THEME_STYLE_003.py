...
class GNAppThemeStyle3(LaunchApp):
    def run(self):
        self.case_module = u"主题风格"  # 用例所属模块
        self.case_title = u'切换为橙色后，查看风格'  # 用例名称
        self.zentao_id = 1989  # 禅道ID
        self.basename = os.path.basename(__file__).split(".")[0]  # 获取用例的文件名称:GNAPP_THEME_STYLE_003
    ...
    def case(self):
        try:
            ...
            self.widget_click(self.page["theme_style_page"]["title"],
                              self.page["theme_style_page"]["orange"],
                              self.page["theme_style_page"]["title"],
                              1, 1, 1, 10, 0.5)
