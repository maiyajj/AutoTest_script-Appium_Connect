# coding=utf-8
class MainPageWidgetIosJD(object):
    # 万能页面
    def god_page(self):
        d = {}
        d["title"] = ["android.widget.FrameLayout", "class", u"万能控件",
                      {"px": {"width": 0, "height": 0}}]
        return d

    def app_home_page(self):
        d = {}
        # 标题
        d["title"] = [u"微联", "name", u"App主页面"]
        # APP主页面标志
        d["app_home"] = [u"微联", "name", u"App主页面"]
        # +号
        d["add_device_plus"] = ["com.jd.smart:id/iv_right", "id", u"+号"]
        return d

    def add_device_method_page(self):
        d = {}
        # 标题
        d["title"] = [u"添加设备", "name", u"添加设备"]
        # 添加设备页面标志
        d["add_device_method"] = [u"添加设备", "name", u"添加设备页面标志"]
        # 通过设备品类添加
        d["add_variety"] = [u"通过设备品类添加", "name", u"通过设备品类添加"]
        # 添加历史
        d["add_history"] = ["com.jd.smart:id/iv_history", "id", u"添加历史"]
        return d

    def add_device_list_page(self):
        d = {}
        # 标题
        d["title"] = [u"设备品类", "name", u"设备品类"]
        # 添加设备页面标志
        d["add_device_list"] = [u"设备品类", "name", u"添加设备页面标志"]
        # 插座列表选项
        d["outlet_option"] = [u"插座", "name", u"插座列表选项"]
        return d

    # driver.find_elements_by_xpath("//android.widget.FrameLayout/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.LinearLayout/android.widget.TextView[@text='2']")
    def add_history_list_page(self):
        d = {}
        # 标题
        d["title"] = [u"设备品类", "name", u"设备品类"]
        # 进入设备添加历史页面标志
        d["add_history_list"] = [u"已添加设备", "name", u"进入设备添加历史页面标志"]
        # 公牛Wi-Fi智能转换器2代电量计量版
        d["hgn_y201J"] = [u"公牛WiFi智能电量统计版转换器2代", "name", u"公牛Wi-Fi智能转换器2代电量计量版"]
        # 公牛Wi-Fi智能转换器2代
        d["hgn_y2011"] = [u"公牛Wi-Fi智能转换器2代", "name", u"公牛Wi-Fi智能转换器2代"]
        # 公牛Wi-Fi智能插座2代加强版
        d["hgn_y2011dl"] = [u"公牛Wi-Fi智能插座2代加强版", "name", u"公牛Wi-Fi智能插座2代加强版"]
        return d

    def add_outlet_list_page(self):
        d = {}
        # 标题
        d["title"] = [u"插座", "name", u"插座"]
        # 进入插座列表页面标志
        d["add_outlet_list"] = [u"插座", "name", u"进入插座列表页面标志"]
        # 公牛Wi-Fi智能插座2代
        d["lgn_y2011"] = [u"公牛Wi-Fi智能插座2代", "name", u"公牛Wi-Fi智能插座2代"]
        # 公牛Wi-Fi智能转换器2代电量计量版
        d["lgn_y201J"] = [u"公牛WiFi智能电量统计版转换器2代", "name", u"公牛WiFi智能电量统计版转换器2代"]
        # 公牛Wi-Fi智能插座2代加强版
        d["lgn_y2011dl"] = [u"公牛Wi-Fi智能插座2代加强版", "name", u"公牛Wi-Fi智能插座2代加强版"]
        return d

    def add_specification_page(self):
        FrameLayout = "/android.widget.FrameLayout"
        LinearLayout = "/android.widget.LinearLayout"
        TextView = "/android.widget.TextView"
        self.xpath = "".join(("/", FrameLayout, FrameLayout, LinearLayout, LinearLayout, TextView))
        TextValue = "[@text='1']"
        self.add_specification = "".join((self.xpath, TextValue))
        d = {}
        # 标题
        d["title"] = [self.add_specification, "xpath", u"搜索设备页"]
        # 搜索设备页
        d["add_specification"] = [self.add_specification, "xpath", u"搜索设备页"]
        # 下一步
        d["add_specification_confirm"] = ["com.jd.smart:id/tv_action", "id", u"下一步"]
        return d

    def input_wifi_password_page(self):
        TextValue = "[@text='2']"
        self.input_wifi_password = "".join((self.xpath, TextValue))
        d = {}
        # 标题
        d["title"] = [self.input_wifi_password, "xpath", u"进入输入密码页面标志"]
        # 进入输入密码页面标志
        d["input_wifi_password"] = [self.input_wifi_password, "xpath", u"进入输入密码页面标志"]
        # 确认wifi密码按钮
        d["confirm_wifi_password"] = ["com.jd.smart:id/tv_action", "id", u"确认wifi密码按钮"]
        # wifi密码输入框
        d["wifi_password"] = ["com.jd.smart:id/tv_pwd", "id", u"wifi密码输入框"]
        return d

    def search_device_loading_page(self):
        TextValue = "[@text='3']"
        self.search_device_loading = "".join((self.xpath, TextValue))
        d = {}
        # 标题
        d["title"] = [self.search_device_loading, "xpath", u"搜索设备等待页面"]
        # 搜索设备等待页面
        d["search_device_loading"] = [self.search_device_loading, "xpath", u"搜索设备等待页面"]
        return d

    def batch_add_device_page(self):
        d = {}
        # 标题
        d["title"] = [u"批量添加", "name", u"设备等待添加"]
        # 设备等待添加
        d["batch_add_device"] = [u"批量添加", "name", u"设备等待添加"]
        return d

    def search_device_success_page(self):
        # 搜索到设备MAC
        self.MAC = {"": 1234}
        self.mac_choose_flag = ""
        self.search_device_mac = self.MAC[self.mac_choose_flag]
        ListView = "//android.widget.ListView"
        LinearLayout = "/android.widget.LinearLayout"
        TextView = "/android.widget.TextView"
        TextValue = "[@text='%s']" % self.search_device_mac
        self.search_device_success = {}
        for i in xrange(1, 5):
            LinearLayout1 = "/android.widget.LinearLayout[%s]" % i
            mac = "".join((ListView, LinearLayout1, LinearLayout,
                           LinearLayout, TextView, TextValue))
            text = "".join((ListView, LinearLayout1, LinearLayout, TextView))

    def search_device_fail_page(self):
        TextValue = "[@text='4']"
        self.search_device_fail = "".join((self.xpath, TextValue))
        d = {}
        # 标题
        d["title"] = [self.search_device_fail, "xpath", u"搜索设备超时"]
        # 搜索设备超时
        d["search_device_fail"] = [self.search_device_fail, "xpath", u"搜索设备超时"]
        return d

    def bind_device_success_page(self):
        d = {}
        # 标题
        d["title"] = ["com.jd.smart:id/btn_config", "id", u"绑定成功页面标志"]
        # 绑定成功页面标志
        d["bind_device_success"] = ["com.jd.smart:id/btn_config", "id", u"绑定成功页面标志"]
        # 确定绑定按钮
        d["confirm_button"] = ["com.jd.smart:id/btn_config", "id", u"绑定成功页面标志"]
        # 输入设备备注名
        d["notes"] = ["com.jd.smart:id/edittext_layout", "id", u"绑定成功页面标志"]
        return d

    def bind_device_page(self):
        d = {}
        # 标题
        d["title"] = [u"该设备已被绑定", "name", u"该设备已被绑定"]
        # 设备已被绑定
        d["bind_device"] = [u"该设备已被绑定", "name", u"该设备已被绑定"]
        return d

    def control_device_page(self):
        d = {}
        # 标题
        d["title"] = ["com.jd.smart:id/i_more", "id", u"设备控制页面标志"]
        # 设备信息进入按钮
        d["device_info_button"] = ["com.jd.smart:id/i_more", "id", u"设备信息进入按钮"]
        # 设备离线标志
        d["offline"] = [u"设备不在线", "name", u"设备离线标志"]
        # 电源开关
        d["power_button"] = ["android.widget.Button", "class", u"电源开关"]
        # 电源开启
        d["power_on"] = [u"设备已开启", "name", u"电源开启"]
        # 电源关闭
        d["power_off"] = [u"设备已关闭", "name", u"电源关闭"]
        # 设备记忆模式
        d["device_memory_mode"] = [u"记忆模式", "name", u"设备记忆模式"]
        # 设备安全模式
        d["device_safe_mode"] = [u"安全模式", "name", u"设备安全模式"]
        return d

    def device_info_page(self):
        d = {}
        # 标题
        d["title"] = [u"设置", "name", u"设备信息页面标志"]
        # 删除设备按钮
        d["unbind_device"] = ["com.jd.smart:id/btn_unbind", "id", u"删除设备按钮"]
        return d


class PopupWidgetIosJD(object):
    # 设备升级确认弹窗
    def update_popup(self):
        d = {}
        d["title"] = ["com.jd.smart:id/cancel", "id", u"有更新"]
        # 检查更新
        d["cancel"] = ["com.jd.smart:id/cancel", "id", u"取消"]
        return d

    def close_ad_popup(self):
        d = {}
        # 广告关闭键
        # d["title"] = [u"操作失败，账号在其他手机登录，请确认是否本人使用。", "name", u"提示 - 重新登录"]
        d["title"] = ["com.jd.smart:id/close_pop_for_top_news", "id", u"发现广告"]
        # 确认
        d["confirm"] = ["com.jd.smart:id/close_pop_for_top_news", "id", u"确认"]
        return d

    def unbind_device_popup(self):
        d = {}
        # 删除设备弹窗
        d["title"] = ["com.jd.smart:id/confirm", "id", u"删除设备按钮", {"text": u"确定要删除设备吗？"}]
        # 确认
        d["confirm"] = ["com.jd.smart:id/confirm", "id", u"确认"]
        # 取消
        d["cancel"] = ["com.jd.smart:id/cancel", "id", u"取消"]

    def bind_device_fail_popup(self):
        d = {}
        # 绑定失败
        d["title"] = ["com.jd.smart:id/confirm", "id", u"绑定失败"]
        # 确认
        d["confirm"] = ["com.jd.smart:id/confirm", "id", u"确认"]
        # 取消
        d["cancel"] = ["com.jd.smart:id/cancel", "id", u"取消"]
        return d

    def loading_popup(self):
        d = {}
        # 标题
        # d["title"] = ["loading...", "name", u"正在加载中loading..."]
        d["title"] = ["android:id/message", "id", u"正在加载中loading..."]
        return d
